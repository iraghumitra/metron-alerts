...
export class MetadataUtil {
     ...
     try {
       ...
       for(let columnName of columnNames) {
         if (seen.indexOf(columnName) === -1) {
           ...
           columnMetadata.push(new ColumnMetadata(columnName, properties[columnName].type ? properties[columnName].type.toUpperCase() : ''));
         }
       }
     } catch (e) {}
  }
  ...
  public static extractData(res:Response): any {
    ...
    for (var index in response.metadata.indices) {
